...
def saveClf(clf,name,name_path,out):
    import joblib
    
    if name == None:
        name = str(clf).split('(')[0]
    
    path_models = os.getcwd()+'/'+out+'/'+name_path+'/models'
    if not os.path.exists(path_models):
        os.mkdir(path_models)
        print("Diretorio " , path_models ,  " criado\n")
# now you can save it to a file
    joblib.dump(clf, path_models+'/'+name+'.pkl') 
    ...
    for d in listDid:
        ...
        if not arg_dataTest:#Verifica se foi passado o dataset de test no input
            ...
            try:
                ...
                X_train, X_test, y_train_label, y_test_label = train_test_split(X, y,test_size=split,random_state=42,stratify=y,train_size=None,shuffle=True)
        else:
        ...
        #Lista de respostas certas
            ...
            saveClf(clf,str(clf).split('(')[0]+'_'+str(i),name_tmp,arg_output)
        ...
        for count, clf in enumerate(list_clf):
            list_accur = []
        ...
        #Lista de respostas certas
            ...
            mlp_resp.append(comparation)
            saveClf(clf,'real_'+str(count),name_tmp,arg_output)
